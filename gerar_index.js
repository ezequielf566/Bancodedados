...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Previews de Imagem/Vídeo */
        .preview-container { width: 100%; height: 200px; background: #000; overflow: hidden; display: flex; align-items: center; justify-content: center; border-bottom: 1px solid #334155; }
        ...
        .filename { font-size: 0.95rem; word-break: break-all; color: #cbd5e1; margin: 0; font-weight: 500; }
...
midias.forEach(arquivo => {
    ...
    // Se for imagem, exibe a tag <img>. Se for vídeo, exibe a tag <video> que roda no hover.
    ...
    html += `
        <div class="card">
            <a href="/${arquivo}" target="_blank" title="Abrir arquivo direto">
...
// 4. Fecha o HTML e adiciona a lógica JavaScript (Client-Side) da página
html += `
    </div>
    ...
    <script>
        ...
        function copiarLink(arquivo) {
            // Monta o link completo dinamicamente pegando o domínio atual
            const url = window.location.origin + '/' + arquivo;
            
            navigator.clipboard.writeText(url).then(() => {
                mostrarToast('✅ Link copiado: ' + arquivo);
            }).catch(err => {
                console.error('Erro ao copiar: ', err);
                alert('Erro ao copiar o link. Seu navegador pode não suportar.');
            });
        }

        // Lógica da animação da notificação
...
// 5. Salva o arquivo fisicamente na raiz para a Vercel servir
